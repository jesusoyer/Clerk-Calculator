// components/Header.tsx
"use client";

import { useState } from "react";

interface HeaderProps {
  onOpenNoteModal: () => void;
  noteLimitReached: boolean;
}

export default function Header({ onOpenNoteModal, noteLimitReached }: HeaderProps) {
  const [open, setOpen] = useState(false);

  return (
    <header className="w-full border-b border-gray-200 bg-white">
      <div className="mx-auto max-w-5xl px-4 py-4 flex items-center justify-between gap-3">
        {/* Left: App title */}
        <span className="text-lg sm:text-xl font-semibold tracking-tight text-gray-900">
          Clerk Calculator
        </span>

        {/* Right: Add note + How to use */}
        <div className="flex items-center gap-2">
          {/* Add note button (to the left of How to use) */}
          <button
            type="button"
            onClick={onOpenNoteModal}
            disabled={noteLimitReached}
            className="
              text-xs sm:text-sm font-medium
              px-3 py-1.5 rounded-md border border-amber-300
              bg-amber-50 text-amber-800
              hover:bg-amber-100 active:scale-95
              transition
              focus:outline-none focus:ring-1 focus:ring-amber-300
              disabled:opacity-50 disabled:cursor-not-allowed
            "
          >
            {noteLimitReached ? "Max 5 notes" : "Add note"}
          </button>

          {/* How to use dropdown */}
          <div className="relative">
            <button
              type="button"
              onClick={() => setOpen((prev) => !prev)}
              className="
                text-xs sm:text-sm font-medium
                px-3 py-1.5 rounded-md border border-gray-300
                bg-white text-gray-700
                hover:bg-gray-100 active:scale-95
                transition
                focus:outline-none focus:ring-1 focus:ring-gray-300
              "
            >
              How to use
            </button>

            {open && (
              <div
                className="
                  absolute right-0 mt-2 w-64 sm:w-80
                  rounded-lg border border-gray-200 bg-white shadow-lg
                  p-3 sm:p-4 text-xs sm:text-sm text-gray-700 z-20
                "
              >
                <p className="mb-2">
                  <span className="font-semibold">Basics:</span> Choose{" "}
                  <span className="font-medium">State jail</span> or{" "}
                  <span className="font-medium">TCJ/TDCJ/ACOP (+1)</span>, then
                  enter your date ranges as <code>MM/DD/YY</code> or just 6–8
                  digits (e.g. <code>010125</code> → <code>01/01/25</code>).
                </p>
                <p className="mb-2">
                  Use <span className="font-medium">Tab</span> or{" "}
                  <span className="font-medium">Enter</span> to move between
                  fields. When you’re on the last end date, pressing Enter will
                  add a new row.
                </p>
                <p className="mb-2">
                  The calculator shows the days for each range, a combined total
                  at the bottom, and a text expression you can{" "}
                  <span className="font-medium">Copy</span> into your paperwork.
                </p>
                <p>
                  To reset everything quickly, press{" "}
                  <span className="font-medium">Ctrl + 0</span> (Windows) or{" "}
                  <span className="font-medium">Cmd + 0</span> (Mac) to clear
                  the calculator.
                </p>
              </div>
            )}
          </div>
        </div>
      </div>
    </header>
  );
}
